...
def avaliacao_iniciar(request, nivel_pk):
    ...
    if ultima and ultima.status in (
        Avaliacao.Status.GERANDO,
        Avaliacao.Status.PRONTA,
        Avaliacao.Status.CORRIGINDO,
    ):
    ...
    avaliacao = Avaliacao.objects.create(
        nivel=nivel,
        tentativa=tentativa,
        status=Avaliacao.Status.GERANDO,
    )
...
def avaliacao_detalhe(request, pk):
    avaliacao = get_object_or_404(
        Avaliacao.objects.select_related("nivel__trilha").prefetch_related("questoes"),
        pk=pk,
...
def avaliacao_resultado(request, pk):
    avaliacao = get_object_or_404(
        Avaliacao.objects.select_related("nivel__trilha").prefetch_related("questoes__resposta"),
        pk=pk,
        nivel__trilha__user=request.user,
    )
    itens = []
...
def exercicio_verificar(request, pk):
    ...
    ex = get_object_or_404(
        ...
        pk=pk,
        lista__nivel__trilha__user=request.user,
    )
...
def revisar_trilha_iniciar(request, trilha_pk):
    ...
    tem_aprovados = Nivel.objects.filter(trilha=trilha, status=Nivel.Status.APROVADO).exists()
    if not tem_aprovados:
