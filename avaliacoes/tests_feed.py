...
class RevisaoRapidaTests(TestCase):
    ...
    def test_lembrei_avanca_o_intervalo(self):
        ...
        self.assertEqual(
            data["proxima_em_dias"], (nivel.revisao_proxima - timezone.localdate()).days
        )
    ...
    def test_rever_reseta_para_amanha(self):
        ...
        self.assertEqual(nivel.revisao_proxima, timezone.localdate() + timezone.timedelta(days=1))
    ...
    def test_quiz_do_dia_some_apos_revisao_de_hoje(self):
        _, _nivel = nivel_aprovado(self.user, devida=False)
...
class RevisaoRenderTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.revisao = Revisao.objects.create(user=cls.user, status=Revisao.Status.PRONTA)
        for i in (1, 2):
            QuestaoRevisao.objects.create(
                ...
                alternativas=[{"letra": "A", "texto": "sim"}, {"letra": "B", "texto": "não"}],
                gabarito="A",
                explicacao_md="Porque sim.",
            )
