...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--dry-run", action="store_true", help="Só mostra o que seria feito, sem salvar"
        )
        parser.add_argument(
            "--auditar",
            action="store_true",
    ...
    def _inserir(self, options):
        ...
        for sub in qs:
            ...
            for i, linhas in enumerate(secoes, start=1):
                trecho = " ".join(
                    linha.strip().lstrip("#").strip() for linha in linhas if linha.strip()
                )[:150]
            ...
            novo = inserir_fotos_conteudo(novo, contexto=f'{trilha.titulo} — tópico "{sub.titulo}"')
            inseridas = novo.count("![")
            if options["dry_run"]:
                self.stdout.write(
                    f"  · {sub.titulo[:50]} — {n} marcador(es), "
    ...
    def _auditar(self, options):
        qs = (
            Subtopico.objects.filter(status=Subtopico.Status.PRONTO, conteudo_md__contains="![")
            .select_related("nivel__trilha")
            .order_by("nivel__trilha_id", "nivel__ordem", "ordem")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Auditoria concluída — {removidas_total} imagem(ns) removida(s).")
        )
