...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--force', action='store_true', help='Refaz também as que já têm emblema')
    ...
    def handle(self, *args, **options):
        ...
        if not trilhas:
            ...
            return
        linhas = [
            f'- id {t.pk}: {t.titulo[:100]}'
            + (f' (área: {t.categoria})' if t.categoria else '')
            for t in trilhas
        ]
        user = (
        ...
        try:
            data = _gerar_json(SYSTEM, user, SCHEMA, None,
