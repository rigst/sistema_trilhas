...
PALETA = {
    ...
    "cabelo_luz": "#4C4790",
    "cabelo": "#332F5E",
    "cabelo_sombra": "#221F42",
    "cabelo_brilho": "#9089DC",
    "sobrancelha": "#2C2950",
    # Roupa
    "camisa_a": "#2AA396",
    "camisa_b": "#4E43B4",
...
PALETAS = (
    ...
    {  # âmbar quente, cabelo castanho
        ...
        "cabelo_luz": "#7A4A2E",
        "cabelo": "#4E2E1E",
        "cabelo_sombra": "#33200F",
        "cabelo_brilho": "#C08A58",
        "sobrancelha": "#3D2416",
    },
    {  # verde/azul frio, pele mais escura
        ...
        "cabelo_luz": "#1F5E6E",
        "cabelo": "#14424E",
        "cabelo_sombra": "#0C2C35",
        "cabelo_brilho": "#5BAFC0",
        "sobrancelha": "#12333C",
    },
    {  # magenta/roxo, cabelo claro
        ...
        "cabelo_luz": "#C9A227",
        "cabelo": "#9A7B18",
        "cabelo_sombra": "#6E570F",
        "cabelo_brilho": "#F0D57A",
        "sobrancelha": "#7A6212",
    },
    {  # azul/índigo, pele escura e cabelo escuro
        ...
        "cabelo_luz": "#2B2B3E",
        "cabelo": "#1B1B2A",
        "cabelo_sombra": "#101018",
        "cabelo_brilho": "#5C5C7A",
        "sobrancelha": "#14141F",
    },
    {  # coral/terra, cabelo ruivo
        ...
        "cabelo_luz": "#C2410C",
        "cabelo": "#9A3412",
        "cabelo_sombra": "#6B240C",
        "cabelo_brilho": "#FB923C",
        "sobrancelha": "#7C2D12",
    },
)
...
Quadro = namedtuple(
    "Quadro",
    "boca olhos cejas corpo_dx corpo_dy cab_dx cab_dy cab_giro",
)
...
def _tamanho() -> int:
    return int(getattr(settings, "VIDEO_AVATAR_TAMANHO", TAMANHO_PADRAO) or TAMANHO_PADRAO)
    ...
    for boca in range(NIVEIS_BOCA):
        for olhos in OLHOS:
            for cejas in CEJAS:
                itens.append(
                    (
                        f"cabeca_b{boca}_o{olhos}_s{cejas}",
                        CAB_RECORTE,
...
def animacao(niveis: list[int], fps: int | None = None, inicio_roteiro: int = 0) -> list[Quadro]:
    ...
    for i in range(total):
        ...
        for tipo, inicio, dur in reacoes:
            ...
            if tipo == "piscada":
                ...
                cejas = (
                    "meio" if u < 0.2 else "alto" if u < 0.75 else "meio" if u < 0.95 else "neutro"
                )
            elif tipo == "aceno":
        ...
        quadros.append(Quadro(boca, olhos, cejas, corpo_dx, corpo_dy, cab_dx, cab_dy, cab_giro))
    return quadros
...
def _compor(anim: list[Quadro], arte: dict[str, str], tam: int, destino: str) -> None:
    ...
    cab_org = (margem + round(CAB_RECORTE[0] * escala), margem + round(CAB_RECORTE[1] * escala))
    ...
    try:
        for q in anim:
            quadro = base.copy()
            desloca_corpo = (
                margem + round(q.corpo_dx * escala),
                margem + round(q.corpo_dy * escala),
            )
            quadro.alpha_composite(corpo, dest=desloca_corpo)
            cabeca = girado(
                cabecas[(q.boca, q.olhos, q.cejas)], (q.boca, q.olhos, q.cejas), q.cab_giro
            )
            quadro.alpha_composite(
                cabeca,
                dest=(cab_org[0] + round(q.cab_dx * escala), cab_org[1] + round(q.cab_dy * escala)),
            )
            ...
            proc.stdin.write(quadro.crop((margem, margem, margem + tam, margem + tam)).tobytes())
        proc.stdin.close()
...
def clipes(audios: list[str], out_dir: str, paleta: dict | None = None) -> list[str] | None:
    ...
    try:
        ...
        saidas = [os.path.join(out_dir, f"mascote_{i:03d}.webm") for i in range(len(audios))]
        ...
        video_utils.em_paralelo(_um, list(range(len(audios))), video_montagem.workers())
        return saidas
    except Exception:
        logger.warning("Mascote do vídeo indisponível; seguindo sem ele.", exc_info=True)
        return None
