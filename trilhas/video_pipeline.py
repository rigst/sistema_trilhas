...
_CATEGORIA_CLIMA = [
    ...
    (
        ("histó", "hist", "direit", "filosof", "saúde", "saude", "medic", "psicolog", "religi"),
        "espiritual",
    ),
...
def gerar_video(video, profile=None, progresso=None):
    ...
    faltantes = [
        s
        for s in nivel.subtopicos.all().order_by("ordem")
    ...
    if faltantes:
        ...
        textos = _ia_em_paralelo(
            lambda s: services.gerar_conteudo_subtopico(s, profile),
            faltantes,
        ...
        for sub, texto in zip(faltantes, textos, strict=False):
    ...
    roteiros = _ia_em_paralelo(
        lambda s: services.gerar_roteiro_video(s, profile),
        subs,
        progresso=lambda pronto, total: _p(
            20 + int(25 * pronto / total), f"Escrevendo o roteiro ({pronto} de {total} tópicos)"
        ),
    )

    # Um capítulo por tópico: capa curta + as seções narradas do conteúdo.
    for i, (sub, roteiro) in enumerate(zip(subs, roteiros, strict=False)):
        if not roteiro:
            continue
        paginas.append(
            {
    ...
    try:
        ...
        slides = video_slides.render_slides(
            paginas, os.path.join(trabalho, "slides"), mascote=video_avatar.ativo()
        )
        ...
        dur = video_montagem.montar(
            ...
            avatares=avatares,
            progresso=lambda pronto, total: _p(
                78 + int(17 * pronto / total), f"Montando o vídeo ({pronto} de {total} trechos)"
            ),
        )
        _p(97, "Finalizando")
    finally:
    ...
    if video.arquivo and video.arquivo != url:
        antigo = os.path.join(settings.MEDIA_ROOT, video.arquivo.replace(settings.MEDIA_URL, "", 1))
