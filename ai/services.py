...
def _foto_mais_aderente(photos: list, query: str, excluir=None):
    ...
    return max(photos, key=lambda p: sum(1 for w in termos if w in (p.get("alt") or "").lower()))
...
def buscar_capa(
    titulo: str, categoria: str = "", descricao: str = "", profile=None, excluir_ids=None
) -> str:
...
def _debitar(profile, usage, model):
    ...
    custo_cache = (
        (Decimal(cr) * Decimal("0.1") + Decimal(cw) * Decimal("1.25"))
        / 1_000_000
        * Decimal(str(pin))
    )
...
def gerar_perguntas_direcionadoras(trilha, profile=None):
    ...
    data = _gerar_json(
        prompts.SYSTEM_PERGUNTAS,
        user,
        prompts.SCHEMA_PERGUNTAS,
        profile,
        model=_model_geral(),
...
def gerar_sumario(trilha, profile=None):
    ...
    data = _gerar_json(
        prompts.SYSTEM_SUMARIO,
        user,
        prompts.SCHEMA_SUMARIO,
        profile,
...
def gerar_roteiro_video(subtopico, profile=None):
    ...
    data = _gerar_json(
        prompts.SYSTEM_ROTEIRO_VIDEO,
        user,
        prompts.SCHEMA_ROTEIRO,
        profile,
        model=_model_geral(),
        effort=getattr(settings, "AI_EFFORT_GERAL", "medium"),
    ...
    for i, secao in enumerate(secoes, start=1):
        ...
        if not narr:
            narr = (subtopico.descricao_curta or "").strip() or secao.splitlines()[0].strip(
                "# "
            ).strip()
...
def gerar_avaliacao(avaliacao, profile=None):
    ...
    for _tentativa in range(2):  # 1 chamada + 1 nova tentativa se vier incompleta
        data = _gerar_json(
            prompts.SYSTEM_AVALIACAO,
            user,
            prompts.SCHEMA_AVALIACAO,
            profile,
            model=_model_geral(),
            effort=getattr(settings, "AI_EFFORT_GERAL", "medium"),
        )
...
def corrigir_avaliacao(avaliacao, profile=None):
    ...
    for questao in avaliacao.questoes.all().prefetch_related("resposta"):
        ...
        resposta.feedback_md = (
            "Resposta correta." if nota else f"Resposta incorreta. Gabarito: **{correta or '—'}**."
        )
    ...
    avaliacao.feedback_geral = f"Você atingiu {nota_final:.1f}. " + (
        "Aprovado! Nível concluído."
        if aprovado
...
def _aprovar_nivel(nivel, ja_aprovado=False):
    ...
    proximo = (
        nivel.trilha.niveis.filter(ordem__gt=nivel.ordem, status=Nivel.Status.BLOQUEADO)
        .order_by("ordem")
        .first()
    )
    if proximo:
...
def gerar_exercicios(lista, profile=None):
    ...
    for _tentativa in range(2):
        data = _gerar_json(
            prompts.SYSTEM_EXERCICIOS,
            user,
            prompts.SCHEMA_EXERCICIOS,
            profile,
            model=_model_geral(),
            effort=getattr(settings, "AI_EFFORT_GERAL", "medium"),
        )
...
def categorizar_trilhas(trilhas, profile=None):
    ...
    data = _gerar_json(
        prompts.SYSTEM_CATEGORIA,
        user,
        prompts.SCHEMA_CATEGORIA,
        profile,
        model=_model_geral(),
        effort=getattr(settings, "AI_EFFORT_GERAL", "medium"),
    )
...
def gerar_percurso(percurso, profile=None):
    ...
    data = _gerar_json(
        prompts.SYSTEM_MENTOR,
        user,
        prompts.SCHEMA_MENTOR,
        profile,
        model=_model_planejamento(),
        effort=getattr(settings, "AI_EFFORT", "high"),
    )
...
def gerar_sugestoes(sessao, profile=None):
    ...
    data = _gerar_json(
        prompts.SYSTEM_SUGESTOES,
        user,
        prompts.SCHEMA_SUGESTOES,
        profile,
        model=_model_planejamento(),
        effort=getattr(settings, "AI_EFFORT", "high"),
    )
...
def gerar_revisao(revisao, profile=None, trilha_id=None):
    ...
    for _tentativa in range(2):
        data = _gerar_json(
            prompts.SYSTEM_REVISAO,
            user,
            prompts.SCHEMA_REVISAO,
            profile,
            model=_model_geral(),
            effort=getattr(settings, "AI_EFFORT_GERAL", "medium"),
        )
