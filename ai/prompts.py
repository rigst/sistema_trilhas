...
SYSTEM_SUBTOPICO = (
    ...
    '- A cada 4-6 seções, inclua uma caixa `!!! quiz "Pergunta direta sobre o que '
    'acabou de ser explicado?"` cujo corpo (indentado) é a resposta com uma breve '
    'explicação — a interface esconde a resposta até o aluno pedir para ver. '
    'É uma checagem de compreensão, não uma prova.\n'
    '- Quando um processo, hierarquia, linha do tempo, ciclo ou relação puder ser '
    'DESENHADA, inclua um diagrama em cerca ```mermaid (flowchart TD ou LR, '
    'mindmap, timeline ou sequenceDiagram) com rótulos curtos em português. '
    'Vise ao menos um diagrama por tópico quando o assunto permitir; o diagrama '
    'fica numa seção própria, entre `---`.\n'
    '- VARIE o formato das seções ao longo do tópico: explicação direta, analogia '
    'com o cotidiano, exemplo de código comentado, tabela comparativa, diagrama, '
    'caixa de destaque, mini-quiz. Nunca emende muitas seções seguidas de '
    'parágrafos puros — alterne os formatos para manter a leitura viva.\n'
    ...
    '- Separe o texto em SEÇÕES CURTAS com uma linha contendo apenas `---` '
    '(o leitor exibe cada seção como um card deslizável, estilo stories). Cada '
    'seção desenvolve UMA ideia: 1-3 parágrafos, OU um exemplo de código com '
    'breve explicação, OU uma caixa de destaque. Nunca coloque `---` dentro de '
    'bloco de código. A profundidade vem de MUITAS seções boas, não de seções '
    'longas.\n\n'
